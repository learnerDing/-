                           ...
                           float             f_beta = 0.0f)
{
    ...
    int is_fp16_computeType = computeType_ == CUDA_R_16F ? 1 : 0; //判断计算的数据类型是否是FP16的
...
//Tensor是4维时(batchsize,channels,height,width),那么这里的stride（步幅）为height*width,batchcount为batchsize*channels
